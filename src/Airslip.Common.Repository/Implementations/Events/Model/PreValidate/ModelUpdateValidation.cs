...
public class ModelUpdateValidation<TEntity, TModel> : IModelPreValidateEvent<TEntity, TModel> 
    where TModel : class, IModel 
    where TEntity : class, IEntity
{
    private readonly IModelValidator<TModel> _validator;

    public ModelUpdateValidation(IModelValidator<TModel> validator)
    {
    ...
    public async Task<List<ValidationResultMessageModel>> Validate(RepositoryAction<TEntity, TModel> repositoryAction)
    {
        ...
        result.AddRange((await _validator.ValidateUpdate(repositoryAction.Model)).Results);
