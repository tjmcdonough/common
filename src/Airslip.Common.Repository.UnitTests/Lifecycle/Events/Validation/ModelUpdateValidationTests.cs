...
public class ModelUpdateValidationTests
{
    [Theory]
    [InlineData(LifecycleStage.Update, "Some name", 0, null)]
    [InlineData(LifecycleStage.Update, "", 1, null)]
    [InlineData(LifecycleStage.Update, "01234567890123456789Invalid", 1, null)]
    [InlineData(LifecycleStage.Create, "Some name", 1, ErrorMessages.LifecycleEventDoesntApply)]
        ...
        string? expectedMessage)
    {
        IModelPreValidateEvent<MyEntity, MyModel> validateEvent =
            new ModelUpdateValidation<MyEntity, MyModel>(new MyModelValidator());
